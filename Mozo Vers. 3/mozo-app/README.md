...
Todo lo visual e interactivo funciona: navegar entre locales, abrir el modal de pedido, sumar/restar cantidades, confirmar un pedido, mover comandas entre columnas en el panel admin, agregar platos al menú, agregar/quitar mesas, iniciar sesión / registrarse, y elegir una ubicación de entrega con mapa (por dirección o con tu ubicación actual).
...
- Los locales y el menú siguen escritos como datos de ejemplo en `js/main.js` (`PLACES`, `MENU`) — no vienen de una base de datos.
- Los pedidos (`orders`) y la cantidad/estado de mesas se guardan en el `localStorage` del navegador, no en un servidor. Esto significa que solo se sincronizan entre pestañas del **mismo navegador en la misma computadora** — un cliente real, en su propio celular, no le llegaría el pedido a un admin en otra computadora. Para eso hace falta el backend.
...
- La ubicación del cliente ya no usa una lista fija de barrios: el usuario ingresa su dirección y la app consulta Nominatim (el servicio de geocodificación gratuito de OpenStreetMap) para mover automáticamente el mapa 2D; también puede usar la ubicación actual del dispositivo con el botón "Usar mi ubicación actual". El mapa usa OpenStreetMap vía Leaflet.js y todo esto necesita conexión a internet.
- Si borrás los datos de navegación del navegador (o usás modo incógnito), los pedidos, mesas y platos agregados se pierden.

## Próximo paso técnico

Cuando quieras que esto sea una app real (con datos que se guardan, usuarios reales, y que el pedido del cliente le llegue de verdad al admin), el siguiente paso es conectar una base de datos como Supabase — tal como definimos en el stack tecnológico de la propuesta. Ahí es también donde iría la autenticación real (hash de contraseñas, envío de mails de verdad) y, si se quiere, un servicio de geocoding pago con más límite de uso (Google Places, Mapbox, etc.) en vez de Nominatim.

## Mejoras implementadas

- **Inicio:** el mensaje debajo de "Mozo." tiene una jerarquía visual coherente con la paleta y las tipografías del proyecto ("Reservá · Pedí · Sentate").
- **Ubicación:** se eliminaron las opciones fijas de barrios. La dirección se busca mediante geocodificación real (Nominatim/OpenStreetMap) y el mapa 2D se mueve automáticamente; también se agregó un botón para usar la ubicación actual del dispositivo.
- **Mesas:** el admin puede aumentar o reducir la cantidad de mesas con `+` y `−`. La configuración queda guardada en `localStorage` y no permite eliminar una mesa ocupada o reservada.
- **Portal cliente → acceso rápido a "Soy un local":** en el top-nav de `cliente.html` ahora hay un menú "⋮" arriba a la derecha con la opción **"Soy un local"**, que lleva directo al panel admin sin tener que volver al inicio.
