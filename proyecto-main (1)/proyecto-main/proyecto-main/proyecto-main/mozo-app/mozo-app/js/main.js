...
if (placeGrid) {
  ...
  if (registerModal) {
    ...
    document.getElementById("registerSubmitBtn").addEventListener("click", () => {
      ...
      saveUserPrefs({ name, email, foodPrefs, carnePunto });
...
if (board) {
  ...
  // Cantidad y estado de mesas: se guarda en localStorage para que
  // quede configurado entre visitas (mock — el día de mañana esto
  // vendría de la base de datos, igual que orders/menu).
  ...
  function renderTables() {
    ...
    if (removeBtn) {
      ...
      removeBtn.disabled = TABLES.length <= 1 || !last || last.status !== "free";
    }
  }
  ...
  if (removeTableBtn) {
    removeTableBtn.addEventListener("click", () => {
      ...
      if (last.status !== "free") {
        showToast(`No podés eliminar ${last.name}: está ${STATUS_LABEL[last.status].toLowerCase()}.`);
        return;
      }
      TABLES.pop();
      saveTables(TABLES);
